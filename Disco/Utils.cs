...
namespace Disco
{
    public class Utils
    {
        ...
        public static bool CheckBanned(SocketMessage message)
        {
            var banReason = "None provided";
            var banType = "";
            bool banned = false;
            foreach (var ban in BanBucket.banList)
            {
                if (ban.User != null && message.Author.Id.ToString() == ban.User)
                {
                    banned = true;
                    if (ban.Reason != null)
                    {
                ...
                if (ban.Server != null && (message.Channel as SocketGuildChannel).Guild.Id.ToString() == ban.Server)
                {
                    banned = true;
                    if (ban.Reason != null)
                    {
            ...
            if (banned)
                SendError(message.Channel, $"You are not permitted to use this feature - this {banType} is banned. Reason: {banReason}");
